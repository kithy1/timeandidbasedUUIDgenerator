...
/**
 * Utility methods to create time-based UUIDs (also known as Version 1 UUIDs) from LocalDateTime in system default timezone and long number
 * Implementation based on com.datastax.driver.core.utils.UUIDs
 */
public class TimeAndIdBasedUUIDGenerator {

    private static final long START_EPOCH = makeEpoch();

    private static final AtomicLong lastTimestamp = new AtomicLong(0L);

    public static UUID timeBased(LocalDateTime localDateTime, Long id) {
        return new UUID(makeMSB(getCurrentTimestamp(millisOf(localDateTime))), id);
    }

    private static long millisOf(LocalDateTime localDateTime){
    ...
    private static long getCurrentTimestamp(Long millis) {
        while (true) {
            long now = fromUnixTimestamp(millis);
            long last = lastTimestamp.get();
            if (now > last) {
                if (lastTimestamp.compareAndSet(last, now)) return now;
            } else {
                long lastMillis = millisOf(last);
                // If the clock went back in time, bail out
                if (millisOf(now) < millisOf(last)) return lastTimestamp.incrementAndGet();
